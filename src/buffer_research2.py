...
def compute_rebalanced_volume(df, init_buffer_size, token_prices_in_usd):
    ...
    buffers_across_time = compute_buffers_constant(df, buffers, token_prices_in_usd)
...
def compute_rebalances_table(df, token_prices_in_usd):
    ...
    for initial_buffer_vol in [2_000_000, 10_000_000, 50_000_000]:
        rebalanced_vol, final_vol = \
            compute_rebalanced_volume(df, initial_buffer_vol, token_prices_in_usd)
        r.append({
...
if __name__ == '__main__':
    ...
    if fetch_data_from_dune:
        ...
        dune_data = dune_connection.fetch(
            query_filepath="./src/dune_api/queries/gnosis_protocol_trades.sql", # NOTE: Check that this query wasn't changed!
            network='mainnet',
            name="dex ag trades",
        )
    ...
    df = df[(df['usd_amount'].notna() & df['sell_amount'].notna() & df['buy_amount'].notna())]

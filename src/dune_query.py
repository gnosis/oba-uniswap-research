...
def run_dune_query_helper(query_id, query_parameters, remember_token_cookie=GNOSIS_ACCOUNT_COOKIE):
...
def run_dune_query(query_id, query_parameters, remember_token_cookie=GNOSIS_ACCOUNT_COOKIE):
    wait_time = 5
    backoff_factor = 2
    while True:
        try:
            return run_dune_query_helper(query_id, query_parameters, remember_token_cookie)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run Dune Analytics query.')
    ...
    parameters = {
        kv.split('=')[0]: kv.split('=')[1] for kv in args.query_parameters
    }

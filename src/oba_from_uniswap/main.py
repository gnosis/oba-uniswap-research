...
def load_swaps(filename):
    ...
    with open(filename, "r") as f:
        ...
        for row in reader:
            ...
            buy_amount, path, output_amounts) = row
...
def swap_to_order(swap, token_info):
    ...
    # represents the xrate as a ratio T1/T2 where T1,T2 are sorted lexicographically
...
def postprocess(csv_filename, tokens_filename, output_dir):
    ...
    for pair, orders in split_by_pair(orders).items():
        with open(output_dir + f'/{pair[0]}-{pair[1]}.json', 'w+') as f:
            json.dump({
                'orders': orders
            }, f, indent=2)
...
parser = argparse.ArgumentParser(
    description='Generate a set of benchmarks from the csv file generated by '
...
parser.add_argument(
    'output_dir',
    type=str,
    help='Path to directory to create instances in.'
)

args = parser.parse_args()

postprocess(args.filename, args.token_file, args.output_dir)
